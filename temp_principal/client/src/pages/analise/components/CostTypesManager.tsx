...
export function CostTypesManager({ costTypes, isDark, refetch }: any) {
    ...
    const handleDeleteOrToggle = async (type: any) => {
        // Se já estiver inativo, permitir reativar
        if (type.isActive === false) {
            if (!confirm("Deseja reativar este tipo de custo?")) return;
            try {
                await api.put(`/financial/cost-types/${type.id}`, { isActive: true });
                refetch();
            } catch (error) {
                console.error(error);
        ...
        // Se ativo, tentar EXCLUIR primeiro
        if (!confirm("Deseja EXCLUIR este tipo de custo? Se houver lançamentos, ele será apenas desativado.")) return;

        try {
            await api.delete(`/financial/cost-types/${type.id}`);
            alert("Tipo de custo excluído com sucesso!");
            refetch();
        } catch (error: any) {
            // Se der erro 409 (Conflict), significa que está em uso
            if (error.response && error.response.status === 409) {
                if (confirm("Este tipo de custo possui lançamentos e não pode ser excluído definitivamente. Deseja apenas DESATIVÁ-LO para impedir novos lançamentos?")) {
                    try {
                        await api.put(`/financial/cost-types/${type.id}`, { isActive: false });
                        refetch();
                        alert("Tipo de custo desativado.");
                    } catch (e) {
                        console.error(e);
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button
                        onClick={async () => {
                            if (!confirm("Deseja importar os tipos de custo do backup? Isso pode duplicar nomes existentes.")) return;
                            setLoading(true);
                            try {
                                const res = await api.post("/financial/cost-types/restore-defaults");
                                refetch();
                                alert(res.data.migrated ? `Importado ${res.data.count} tipos do backup!` : "Nenhum dado legado encontrado. Padrões mantidos.");
                            } catch (error) {
                                console.error(error);
                        ...
                        style={{
                            ...
                            color: isDark ? "#94a3b8" : "#64748b", background: "transparent", fontSize: "0.875rem", fontWeight: "500", cursor: "pointer"
                        }}
                    >
                        <Database size={16} /> Importar Legado
                    </button>
                    <button
                ...
                {costTypes.length === 0 && (
                    <div style={{
                        ...
                        <p style={{ marginBottom: "1.5rem" }}>
                            Cadastre um novo tipo de custo para começar.
                        </p>
                    </div>
                )}
                    ...
                    return (
                        <div key={type.id} style={styles.card}>
                            ...
                            <div style={{ display: 'flex', gap: '0.5rem' }}>
                                ...
                                <button
                                    title={type.isActive !== false ? "Excluir / Desativar" : "Reativar"}
                                    onClick={() => handleDeleteOrToggle(type)}
                                    style={{
