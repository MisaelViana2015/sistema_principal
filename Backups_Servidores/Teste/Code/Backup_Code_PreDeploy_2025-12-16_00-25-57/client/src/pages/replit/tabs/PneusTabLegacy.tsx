...
export default function PneusTabLegacy() {
    ...
    useEffect(() => {
        // Carregar Veículos
        loadVehicles().catch(err => console.error("Falha ao carregar veículos:", err));

        // Carregar Pneus (passando array vazio de veiculos inicialmente se não tiver carregado ainda, 
        // mas idealmente loadPneus busca tudo e depois fazemos o match no render ou num useEffect separado se precisassemos dos veiculos
        // Mas o loadPneus atual PRECISA dos veiculos para fazer o map.
        // Vamos alterar a estrategia: loadPneus busca os dados, e um useEffect separado faz o join, OU loadPneus busca os veiculos de novo se precisar?
        // Melhor: loadPneus chama a API, guarda no state 'rawPneus', e um useMemo faz o join com 'vehicles'.
        // Mas para manter simples e funcionando AGORA:

        // Vamos carregar pneus independentemente e tentar fazer o join com o que tiver, ou esperar.
        // O problema atual é: loadPneus recebe 'currentVehicles'.

        // Vamos chain ainda, mas com proteção.
        vehiclesService.getAll()
            ...
            .catch(err => {
                ...
                // Tenta carregar pneus mesmo sem veículos para não mostrar tela vazia (nomes ficarão N/A)
                loadPneus([]);
            });
