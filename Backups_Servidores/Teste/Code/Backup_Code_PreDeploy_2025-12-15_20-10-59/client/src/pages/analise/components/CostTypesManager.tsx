...
export function CostTypesManager({ costTypes, isDark, refetch }: any) {
    ...
    const handleFixVisuals = async () => {
        if (!confirm("Isso irá aplicar ícones e cores padrão a todos os tipos de custo antigos que não possuem configuração visual. Deseja continuar?")) return;
        setLoading(true);
        try {
            await api.post("/financial/fix-legacy-visuals");
            alert("Ícones e cores atualizados com sucesso!");
            refetch();
        } catch (error) {
            console.error(error);
            alert("Erro ao atualizar visuais.");
        } finally {
            setLoading(false);
        }
    };
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button
                        onClick={handleFixVisuals}
                        title="Restaurar ícones padrão para itens antigos"
                        style={{
                            display: "flex", alignItems: "center", justifyContent: 'center',
                            padding: "0.5rem", borderRadius: "0.375rem", border: `1px solid ${isDark ? "#334155" : "#e2e8f0"}`,
                            color: isDark ? "#94a3b8" : "#64748b", background: "transparent", cursor: "pointer"
                        }}
                    >
                        <Wand2 size={16} />
                    </button>
