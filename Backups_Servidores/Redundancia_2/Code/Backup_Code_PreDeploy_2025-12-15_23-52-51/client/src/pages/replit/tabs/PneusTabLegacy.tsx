...
export default function PneusTabLegacy() {
    ...
    const availableYears = Array.from(new Set(pneus.map(p => new Date(p.installDate).getFullYear()))).sort((a, b) => b - a);
    ...
    const filteredPneus = pneus.filter(p => {
        ...
        if (selectedVehicle !== "todos" && p.veiculoPlate !== selectedVehicle) {
            // selectedVehicle value in select is v.plate (line 173)
            return false;
        }
