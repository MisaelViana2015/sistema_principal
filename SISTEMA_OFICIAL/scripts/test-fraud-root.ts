...
import { FraudService } from "./server/modules/fraud/fraud.service.js";
import { desc, eq } from "drizzle-orm";

async function analyzeRecentShifts() {
    console.log("🔍 Iniciando análise de fraudes em turnos recentes (ROOT VERSION)...");
    ...
    for (const shift of recentShifts) {
        ...
        try {
            ...
            if (result.riskLevel !== 'low') {
                ...
                console.log(`      Regras:`);
                result.rules.forEach((r: any) => console.log(`        - [${r.severity}] ${r.label}`));
            } else {
