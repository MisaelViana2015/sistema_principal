...
// Data from user - December 16-17, 2025
const shiftsData = [
    // 16/12/2025
    {
        driver: "Robson",
...
async function importShifts() {
    console.log("🚀 Iniciando importação de turnos...\\n");

    for (const shiftData of shiftsData) {
        try {
            // Find driver
            const [driver] = await db.select().from(drivers).where(eq(drivers.nome, shiftData.driver));
            if (!driver) {
                console.log(`❌ Motorista "${shiftData.driver}" não encontrado. Pulando...`);
                continue;
            }

            // Find vehicle
            const [vehicle] = await db.select().from(vehicles).where(eq(vehicles.plate, shiftData.vehicle));
            if (!vehicle) {
                console.log(`❌ Veículo "${shiftData.vehicle}" não encontrado. Pulando...`);
                continue;
            }

            // Create shift
            const startTime = new Date(`${shiftData.date}T${shiftData.start}:00-03:00`);
            const endTime = new Date(`${shiftData.date}T${shiftData.end}:00-03:00`);
            ...
            const repasseEmpresa = totalBruto * 0.6;
            const repasseMotorista = totalBruto * 0.4;
            ...
            const [newShift] = await db.insert(shifts).values({
                ...
                totalCustos: 0,
                liquido: totalBruto,
                repasseEmpresa: repasseEmpresa,
                repasseMotorista: repasseMotorista,
            }).returning();

            console.log(`✅ Turno criado: ${driver.nome} - ${vehicle.plate} (${shiftData.date})`);

            // Create rides
            for (const rideData of shiftData.rides) {
                const rideTime = new Date(`${shiftData.date}T${rideData.time}:00-03:00`);

                await db.insert(rides).values({
                    shiftId: newShift.id,
                    tipo: rideData.type,
                    valor: rideData.value,
                    timestamp: rideTime,
                });
            }

            console.log(`   📍 ${shiftData.rides.length} corridas adicionadas\\n`);

        } catch (error) {
            console.error(`❌ Erro ao importar turno de ${shiftData.driver}:`, error);
        }
    }
...
importShifts().catch(console.error);
