...
export default function TurnoPage() {
    ...
    const [workedTime, setWorkedTime] = useState("--h --min"); // New state for timer
    ...
    async function loadRides(shiftId: string) {
        try {
            ...
            const data = await ridesService.getAll({
                limit: 20, // Increased limit for dashboard list
                driverId: String(user?.id),
                shiftId: shiftId // Filter by shiftId strictly
            });
            setRides(data.data);
        } catch (err) {
    ...
    async function loadExpenses(shiftId: string) {
        try {
            // Expenses endpoint might not simulate 'getAll' nicely, let's try strict shift filter
    ...
    const getCostTypeStyle = (type: any) => {
        ...
        if (name.includes('outros')) return { color: '#f97316', bg: '#fff7ed', border: '#fdba74', icon: <DollarSign /> };
        if (name.includes('pedágio')) return { color: '#16a34a', bg: '#f0fdf4', border: '#86efac', icon: <DollarSign /> };
        if (name.includes('recarga app')) return { color: '#2563eb', bg: '#eff6ff', border: '#93c5fd', icon: <Wallet /> };
        if (name.includes('carro') || name.includes('abastecimento')) return { color: '#16a34a', bg: '#f0fdf4', border: '#86efac', icon: <Car /> };
        if (name.includes('alimentação')) return { color: '#db2777', bg: '#fce7f3', border: '#fbcfe8', icon: <User /> };
        if (name.includes('manutenção')) return { color: '#dc2626', bg: '#fef2f2', border: '#fca5a5', icon: <DollarSign /> };
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1rem' }}>
                                    ...
                                    {rides.filter(r => r.tipo === 'APP').map((r, i) => (
                                        <div key={r.id} className="flex justify-between items-center text-sm border-b border-gray-100 dark:border-gray-800 pb-1">
                                            <div className="flex gap-2 text-gray-500">
                                                <span>{i + 1}</span>
                                                <span>{new Date(r.hora).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                            </div>
                        ...
                        <div style={cardStyle}>
                            ...
                            {expenses.length === 0 ? <p className="text-sm text-gray-500 dark:text-gray-400">Nenhum custo registrado</p> :
                                expenses.map(e => {
                                    // Use API fields: tipo, valor, data
                                    const typeName = e.tipo || 'Despesa';
                                    const style = getCostTypeStyle({ name: typeName });
            ...
            <MainLayout>
                <div style={{ maxWidth: '600px', margin: '0 auto', padding: '1rem', paddingBottom: '200px' }}>
                    ...
                    <div className="space-y-3 mb-8">
                        ...
                        {/* Motorista - Purple */}
                        <div style={{ backgroundColor: '#9333ea', color: 'white' }} className="rounded-lg p-4 flex justify-between items-center shadow-sm">
                            <div className="flex items-center gap-3 font-bold">
                    ...
                    <div style={cardStyle}>
                        <h3 style={{ fontWeight: 'bold', marginBottom: '1rem' }}>Dados Operacionais</h3>

                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
                            <div>
                                <label style={{ fontSize: '0.875rem', fontWeight: 'bold', display: 'block', marginBottom: '0.25rem' }}>KM Inicial</label>
                                <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>{activeShift.kmInicial}</div>
                            </div>
                            <div>
                                <label style={{ fontSize: '0.875rem', fontWeight: 'bold', display: 'block', marginBottom: '0.25rem' }}>KM Final <span className="text-red-500">*</span></label>
                                <input
                                    type="number"
                                    inputMode="decimal"
                                    value={kmFinal}
                                    ...
                                    placeholder="Digite"
                                    style={{ width: '100%', padding: '0.5rem', borderRadius: '4px', border: `1px solid ${colors.border}`, fontWeight: 'bold', fontSize: '1.1rem', backgroundColor: colors.bg, color: colors.text }}
                                />
                            </div>
                        </div>

                        <div style={{ marginBottom: '1.5rem' }}>
                            <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 'bold' }}>Senha <span className="text-red-500">*</span></label>
                            {/* Hack to prevent autofill */}
                            <div style={{ height: 0, overflow: 'hidden', position: 'absolute' }}>
                                <input type="email" name="fakevallemail" tabIndex={-1} />
                                <input type="password" name="fakevallpass" tabIndex={-1} />
                            </div>
                            <input
                                type="password"
                                ...
                                autoComplete="off"
                                name="finish_password_field_random_123"
                                readOnly={true}
                            ...
                            />
                            <p className="text-xs text-gray-500 mt-1">Por segurança, confirme sua senha antes de encerrar</p>
                        </div>

                        {/* Live Operational Stats */}
                        <div className="space-y-3 pt-4 border-t border-gray-100">
                            <div className="flex justify-between items-center">
                                <div className="flex items-center gap-2 text-sm text-gray-600"><Clock size={16} /> KM Rodados</div>
                                <div className="font-bold">{kmRodados > 0 ? kmRodados : '--'} km</div>
                            </div>
                            <div className="flex justify-between items-center">
                                <div className="flex items-center gap-2 text-sm text-gray-600"><DollarSign size={16} /> Valor por KM</div>
                                <div className="font-bold">R$ {valorPorKm.toFixed(2).replace('.', ',')}</div>
                            </div>
                            <div className="flex justify-between items-center">
                                <div className="flex items-center gap-2 text-sm text-gray-600"><Car size={16} /> Total de Corridas</div>
                                <div className="font-bold">{totalCorridas}</div>
                            </div>
                            <div className="flex justify-between items-center">
                                <div className="flex items-center gap-2 text-sm text-gray-600"><DollarSign size={16} /> Ticket Médio Geral</div>
                                <div className="font-bold">R$ {ticketMedio.toFixed(2).replace('.', ',')}</div>
                            </div>
                            <div className="flex justify-between items-center pt-2 mt-2 border-t border-gray-50 dark:border-gray-800">
                                <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400"><Clock size={16} /> Duração do Turno</div>
                        ...
                        <button
                            ...
                            disabled={!kmFinal || Number(kmFinal) < Number(activeShift.kmInicial) || !finishPassword}
                            style={{
                                width: '100%',
                                padding: '1rem',
                                backgroundColor: (!kmFinal || Number(kmFinal) < Number(activeShift.kmInicial) || !finishPassword) ? '#9ca3af' : '#ef4444',
                                color: 'white',
                                fontWeight: 'bold',
                                borderRadius: '8px',
                                border: 'none',
                                cursor: (!kmFinal || Number(kmFinal) < Number(activeShift.kmInicial)) ? 'not-allowed' : 'pointer',
                                marginTop: '1.5rem'
                            }}
                        >
    ...
    return (
        <MainLayout>
            <div style={{ maxWidth: '600px', margin: '0 auto', paddingTop: '2rem', paddingBottom: '120px' }}>
                <div style={cardStyle}>
                    <h2 style={{ fontSize: '1.5rem', fontWeight: 'bold', marginBottom: '1.5rem', color: colors.text, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <Play className="text-green-500" /> Iniciar Turno
                    </h2>

                    <div style={{ marginBottom: '1.5rem' }}>
                        <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 'bold', color: colors.text }}>Veículo *</label>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.75rem', maxHeight: '280px', overflowY: 'auto', paddingRight: '0.25rem' }}>
                            {vehicles.map((vehicle) => (
                                <button
                                    key={vehicle.id}
                                    onClick={() => vehicle.isActive && setSelectedVehicleId(String(vehicle.id))}
                                    disabled={!vehicle.isActive}
                                    style={{
                                        width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '1rem', borderRadius: '8px',
                                        border: selectedVehicleId === String(vehicle.id) ? `2px solid ${colors.blue}` : `1px solid ${colors.border}`,
                                        backgroundColor: selectedVehicleId === String(vehicle.id) ? (isDark ? '#1e3a8a' : '#eff6ff') : colors.bg,
                                        opacity: vehicle.isActive ? 1 : 0.6, cursor: vehicle.isActive ? 'pointer' : 'not-allowed',
                                        position: 'relative'
                                    }}
                                >
                                    ...
                                    <Car size={28} className={selectedVehicleId === String(vehicle.id) ? "text-blue-500" : "text-gray-400"} />
                                    <div style={{ textAlign: 'center' }}>
                                        <div style={{ fontWeight: 'bold', color: colors.text, fontSize: '1rem' }}>{vehicle.plate}</div>
                                        <div style={{ fontSize: '0.75rem', color: colors.textMuted, marginTop: '2px' }}>{vehicle.modelo}</div>
                                    </div>
                                </button>
                            ))}
                        </div>
                    ...
                    <button
                        ...
                        style={{ width: '100%', padding: '1rem', backgroundColor: colors.green, color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}
                    >
                        Iniciar Turno
                    </button>
                </div>

                {/* Reopen Shift Placeholder */}
                <div style={{ ...cardStyle, marginTop: '1.5rem', opacity: 0.7 }}>
                    <h3 style={{ fontWeight: 'bold', color: colors.text, display: 'flex', gap: '0.5rem' }}><RotateCcw size={20} /> Reabrir Último Turno</h3>
                    <p style={{ fontSize: '0.875rem', color: colors.textMuted, margin: '0.5rem 0 1rem 0' }}>Fechou sem querer? Reabra o último turno encerrado</p>
                    <button disabled style={{ width: '100%', padding: '0.75rem', backgroundColor: colors.red, color: 'white', border: 'none', borderRadius: '8px', cursor: 'not-allowed' }}>Reabrir Último Turno</button>
                </div>
            </div>
        </MainLayout>
    );
